...
import { Check, ArrowRight, RotateCcw, Share2 } from "lucide-react";

// -----------------------------
// H‑CODE QUIZ APP (single file)
// -----------------------------
// Notes:
// - TailwindCSS classes are used for styling.
// - Uses framer-motion for subtle animations.
// - All content is self-contained: items, scoring, and result text.
// - No external chart lib; simple horizontal bars are rendered.
// - Feel free to split into files in your codebase.

// 24 scored items: 6 per axis (3 per pole)
...
// Axis definitions and letter order
const AXES = [
...
function computeScores(answers: Record<number, number>) {
  ...
  ITEMS.forEach((item) => {
    ...
    // For scoring, keep raw polarity; we'll resolve to letters later
    sums[item.axis][item.pole] += v; // higher agreement supports the pole stated
  });
  ...
  AXES.forEach((axis) => {
    ...
    // normalize to 0..100 per pole (simple min‑max: 3 items * 1..5 scale => 3..15 range)
    const min = 3, max = 15;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex flex-wrap gap-3 justify-center">
        ...
        >
          <Share2 className="w-4 h-4" /> Copy share text
        </button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white text-gray-900">
      <div className="max-w-4xl mx-auto p-6">
        <AnimatePresence mode="wait">
          ...
          {step === "result" && result && (
            <motion.section key="result" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
